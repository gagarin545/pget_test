...
class get_incident {
    ...
    static Array get_worker(ClientThread o) throws SQLException {
        ...
        System.out.println(o.getT().getName());
        if(rs.next()) {
    ...
    static void registration(String name, ClientThread client) throws SQLException { // сообщение , клиент
        ...
        stmt.execute(format ("select worker_record(%s,'%s', '%s');" , client.getT().getName(), name.substring(0,name.indexOf('-')), name.substring(name.indexOf('-') + 1) ));
        con.commit();
    }
